...
class media_database:
    ...
    def add_entry(self,new_entry,*args,**kwargs):
        ...
        for a in list(new_entry.attrib.keys()):
            if not a in self.alist:
                self.alist[a] = 1
            else:
                self.alist[a] = self.alist[a] + 1
    ...
    def delete_entry(self,entry):
        ...
        for a in list(entry.attrib.keys()):
            self.alist[a] = self.alist[a] - 1
            if self.alist[a] <= 0:
                self.alist.pop(a)
    ...
    def get_attrib_stat(self):
        ...
        empty = 0
        for i in self.dlist:
            if i.type in attrib['Type']:
                attrib['Type'][i.type] += 1
            else:
                attrib['Type'][i.type] = 1

            entryattrib = i.attrib
            ekeys = list(entryattrib.keys())
            for j in ekeys:
                if j not in attrib:
                    ...
                    for k in entryattrib[j]:
                        if k in attrib[j]:
                            ...
                            attrib[j][k] = 1
            for j in list(attrib.keys()):
                if j != 'Type' and j not in entryattrib:
                    attrib[j]['undefined'] += 1
...
class media_database_sql:
    ...
    def update(self,ty='unknown'):
        ...
        unknown = [x for x in unknown if x not in todelete]
    ...
    def get_selection(self,*args,**kwargs):
        ...
        if len(results) == 0:
            ...
            for check in results[1:]:
                res = [x for x in res if x in check]
        else:
    ...
    def execute_selector(self,cursor,generator,attribute,words,mode,table,isstring):
        ...
        if mode == 'AND' and isstring:
            ...
            buf = res[0]
            for check in res[1:]:
                buf = [x for x in buf if x in check]
        else:
