...
class media_entry:
    ...
    def match_selection(self,type='',style='',name='',case_sensitive=False,
                        ignore_empty=True,crossmatch=False,**kwargs):
        """
        ...
        for i in keys:
            ...
            if len(args) == 0:
                if ignore_empty:
                    continue
                elif i not in self.attrib or len(self.attrib[i]) == 0:
                    continue
                else:
                    return False
            if i in self.attrib:
    ...
    def execute(self,filepath='unknown',singleMode=False):
        ...
        if os.name == 'nt':
            ...
            #prepare the path to be pasted into bash
...
class video_entry(media_entry):
    ...
    def __init__(self,
        ...
        for f in self.filepath:
            try:
                #f = f.encode(encoding) #not needed for python3
...
def convert_to_new_version(entry):
    ...
    try:
        ...
        version = '0.0' #the first version did not have a version label yet
